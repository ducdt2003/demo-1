...
class MovieAppApplicationTests {
    ...
    void save_favorites() {
        ...
        // Lấy danh sách tất cả User và Movie từ database
        List<User> users = userRepository.findAll();
        List<Movie> movies = movieRepository.findAll();

        for (Movie movie : movies) {
            // Sinh ra số lượng favorites ngẫu nhiên cho mỗi phim (từ 10 đến 20)
            int favoriteCount = rd.nextInt(11) + 10;
            for (int i = 0; i < favoriteCount; i++) {
                // Chọn ngẫu nhiên một user
                User randomUser = users.get(rd.nextInt(users.size()));

                // (Tùy chọn) Kiểm tra nếu dữ liệu của favorites đã có cho user-movie này thì bỏ qua
                if (!favoriteRepository.findByUserIdAndMovieId(randomUser.getId(), movie.getId()).isEmpty()) {
                    continue;
                }

                // Tạo một đối tượng Favorite mới, lưu thời gian tạo và liên kết movie, user
